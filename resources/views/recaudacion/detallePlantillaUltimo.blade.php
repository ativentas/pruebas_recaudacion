...
{{csrf_field()}} 
        <table class="table table-striped tg">
            ...
            @foreach($lineas as $linea)
                <tr data-id="{{$linea->id}}">
                    ...
                    <td> {{ $linea->maquina }}</td>
                    <td>
                    <input class="dinero" type="text" min="0" max="9999" step="" tabindex="" name="monedas{{$linea->id}}" pattern="[0-9]+([,\.][0-9][0-9])?" id="monedas{{$linea->id}}" size="" placeholder="" align="" value={{$linea->monedas}} style="width: 4em;" @if($linea->verificado==1) disabled @endif>
                    </td>
            <!-- billetes -->
                    <td>
                    ...
                    <td>
                    <!-- <label id="billetes{{$linea->id}}"></label> -->
                    <input class="dinero" type="text" min="0" max="9995" step="5" tabindex="" name="billetes{{$linea->id}}" pattern=[0-9](?=(?:[0-9]{3})+(?![0-9])) id="billetes{{$linea->id}}" size="" placeholder="" align="" value={{$linea->billetes}} style="width: 4em;" disabled readonly="true">
                    </td>
            ...
            <!-- aquí van las recaudaciones que marcan las máquinas -->
                    <td>
                    <input class="dineroI" type="number" min="0" max="9999" step="0.01" tabindex="" name="monedasI{{$linea->id}}" pattern="[0-9]+([,\.][0-9][0-9])?" id="monedasI{{$linea->id}}" size="" placeholder="" align="" value={{number_format($linea->monedasI)}} style="width: 5em;" @if($linea->verificado=='1') disabled @endif>
                    </td>
                    <td>
        ...
        <script>
            $(document).ready(function() {
                ...
                $("#monedas{{$linea->id}}").change(function() {                              
                    ...
                    $("#ldif{{$linea->id}}:not(:contains('-'))").attr('style','color:#669');
                });
        // Cuando cambia el total de una linea de billetes.OK
            ...
            <tfoot>
                <tr>
                <td colspan=9></td>
                <td><label id="lTOTAL">{{ number_format($plantilla['total'],2,',','.') }}</label>
                ...
                <td colspan=2></td>
                <td><label id="lTOTALI">{{ number_format($plantilla['totalI'],2,',','.') }}</label>
                <input form="form_guardar" type="hidden" id="TOTALPlantillaI" name="TOTALPlantillaI" value="{{$plantilla['totalI']}}" readonly="readonly"></td>
                <td>
                <label class="signo" id="ldiferencia">{{ number_format($plantilla['diferencia'],2,',','.') }}</label>
...
//cuando se introducen los datos de las monedas
    $('.dinero').change(function(){
        ...
        $('#lTOTAL').text(sum);
        var TOTALI = parseFloat($('#TOTALPlantillaI').val());
